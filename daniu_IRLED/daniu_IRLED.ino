...
void setup() {
  ...
  // 原有紅外與風扇控制 API 都不變
  server.on("/on", HTTP_GET, [](AsyncWebServerRequest *request){ fan1 = 1;irsend.sendNEC(0xFF02FD, 32); request->send(200, "text/plain", "暖氣已開啟或關閉"); });
  server.on("/off", HTTP_GET, [](AsyncWebServerRequest *request){ fan1 = 0;irsend.sendNEC(0xFF02FD, 32); request->send(200, "text/plain", "暖氣已開啟或關閉"); });
