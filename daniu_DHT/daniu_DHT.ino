...
void setup() {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    html += "<button class='button button6' onclick=\"sendRequest('onauto')\">開啟自動模式</button>";
    html += "<button class='button button7' onclick=\"sendRequest('offauto')\">關閉自動模式</button>";
  ...
  // 原有紅外與風扇控制 API 都不變
  server.on("/on", HTTP_GET, [](AsyncWebServerRequest *request){ irsend.sendNEC(0xFF02FD, 32); request->send(200, "text/plain", "暖氣已開啟或關閉"); });
  ...
  if (autoMode == 1) {
    ...
    if (!isnan(temperature)) {
      if (temperature >= thresholdTemphigh && fan1 == 0) {
        digitalWrite(RELAY_PIN, HIGH); fan1 = 1;
        Serial.println("自動模式：溫度過高，風扇開啟");
      } else if (temperature < thresholdTemplow && fan1 == 1) {
        digitalWrite(RELAY_PIN, LOW); fan1 = 0;
        Serial.println("自動模式：溫度下降，風扇關閉");
      }
    }
  }
